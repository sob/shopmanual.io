...
| **Fwd Dist Bus (master)** | Mechanical Products<br/>([174-S2-100-2][mp-100], {{ tbd(135) }}) | 100A | Manual | START battery+<br/>└→ 100A CB<br/>&nbsp;&nbsp;&nbsp;└→ [START+ Forward Distribution Bus][front-battery] | ~108A peak (~68A continuous) | Protects 2 AWG forward feed (110A @ 60°C) — downsized from 150A on 2026-09-22 (keep the wire, drop the breaker); the brief ~108A peak sits inside the Series 17 trip curve (135% for 30 s); selective with downstream load CBs |
...
**Total battery-side Circuit Breakers:** 3 (PMU 250A, Forward Dist Bus master 100A, BCDC 80A)
...
The three loads relocated off the PMU each get a breaker at the [START+ Forward Distribution Bus][front-battery] (engine bay), fed from the 100A master above. SKUs follow the Mechanical Products Series 17 S2 pattern — confirm via {{ tbd(135) }}.
...
**Space Requirements:** 3 battery-side CBs (PMU 250A, Forward Bus master 100A, BCDC 80A) side-by-side with wiring clearance: ~10" × 4" (~40 sq in). The 3 forward-bus load CBs (fan/iBooster/TCU) mount in the engine bay on a bracket beside the [Forward Distribution Bus][front-battery], not in the wheel well.
