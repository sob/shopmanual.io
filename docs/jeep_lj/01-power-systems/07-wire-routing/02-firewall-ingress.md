...
*Rationale for the upsize decision. Current pin assignment and connector spec are documented above.*

The original HDP24-24-21 (17 size-16 + 4 size-12 contacts) was at 16/17 size-16 capacity. Five pending circuits (SwitchPros forward outputs, plus two Boomerang keyless signals later dropped when PBS-I replaced Boomerang) would have filled every size-16 cavity with zero headroom, forcing the reserved size-12 cavities into service via reducer crimps.

**Decision:** upsize to HDP24-24-29 (29 size-16 contacts, same 1.5" shell/hole, same HDT-48-00 crimp tool) for ~$20 more than the 24-21, buying 8 spare pins instead of 0. A split-connector alternative (small secondary connector for keyless signals only) was rejected as two penetrations/sealing surfaces for no benefit.
