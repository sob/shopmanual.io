...
through a **200 A SGP32 relay** switched by ignition.
...
**Rear Powerswitch (A) and (B)**. Its coil is on **ACC (ignition)**, so both
rear panels are only live with the key on.
...
| Coil trigger | ACC (ignition) |
| Main feed fuse | **TBD** |
| Feed conductor | **TBD** - gauge and run length from the engine bay to the rear |

!!! warning "Key-off means no rear accessories"
    With the relay on ACC, the rear panels drop when the ignition is off.
    That includes the **Starlink supply and the UXG Lite** on Rear
    Powerswitch (A) circuit 6. Starlink will not run with the van parked
    and the key out unless the trigger changes.
...
- [ ] Decide whether the rear panels should stay live key-off (e.g. for Starlink while parked); if so, change the SGP32 trigger
